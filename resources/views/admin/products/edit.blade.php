...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Редактирование {{ $product->name }}</h1>
                </div>
            </div>
        </div>
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="{{ route('products.update', $product) }}" method="POST" class="col-md-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-group">
                        ...
                        >
                            ...
                            @foreach(\App\Models\Product::CATEGORIES as $key => $category)
                                <option value="{{ $key }}" @if($product->category_id == $key) selected @endif>{{ $category }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description"
                                  ...
                                  rows="10"
                        >{{ old('description', $product->description) ? trim(old('description', $product->description)) : '' }}</textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div id="thumbnail-preview"><img class="img-thumbnail" src="{{ $product->thumbnailUrl }}" alt=""></div>
